...
```
```{R}
bind_rows(mx,px) %>% arrange(Species,Vicinity, `Tributary/Collection Location`) %>% separate(Ind, sep="-", into=c("Plate","PlateNo","Barcode")) %>% mutate(Forward=paste0(Plate,"-",PlateNo,"_RA_",Barcode,".fastq")) %>%
  mutate(Reverse=paste0(Plate,"-",PlateNo,"_RB_",Barcode,".fastq")) %>% write_csv("genbank/sequences-to-submit-09102025.csv")
```
