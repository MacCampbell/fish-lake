...
Getting all populations sequenced (30) + ogs for genbank
DDP12_09 DDP13_13

%>% filter(`Tributary/Collection Location` %in% c("NN4","NV12","Tui Slough", "Camp Cady")) %>% filter(Sample != "237_1")
```{r}
mx<-read_csv("meta/04152023.csv") %>% mutate(Path=paste0("data/align/",Ind,".sort.flt.bam")) %>% 
   mutate(Path2=ifelse(Dedup > 2e6, paste0("data/downsample/",Ind,".reduced.bam"), Path)) %>% filter(`Tributary/Collection Location` %in% c("NN4","NV12","Tui Slough", "Camp Cady")) %>% filter(GVL_Code != "TSL11_05")
mx

px <- read_csv("meta/05012024.csv") %>% mutate(Path=paste0("data/align/",Ind,".sort.flt.bam")) %>% 
   mutate(Path2=ifelse(Dedup > 2e6, paste0("data/downsample/",Ind,".reduced.bam"),
                     Path)) %>% filter(`Tributary/Collection Location` %in% phylo$`Tributary/Collection Location`) %>% filter(!`Tributary/Collection Location` %in% c("NN4","NV12","Tui Slough", "Camp Cady"))

```
```{R}
dmpx<-bind_rows(mx,px) %>% arrange(Species,Vicinity, `Tributary/Collection Location`) %>% separate(Ind, sep="-", into=c("Plate","PlateNo","Barcode")) %>% mutate(Forward=paste0(Plate,"-",PlateNo,"_RA_",Barcode,".fastq")) %>%
  mutate(Reverse=paste0(Plate,"-",PlateNo,"_RB_",Barcode,".fastq")) %>% relocate(Forward,Reverse)

dmpx%>% write_csv("genbank/sequences-to-submit-09102025.csv")
```

data of the form plate-1_RA_GGGTCGTAGATGCAGG.fastq		
