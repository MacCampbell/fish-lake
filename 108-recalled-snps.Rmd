...
```
```{r}
ml<-read.tree("outputs/108/pruned-429-maf01.min4.phy.varsites.phy.contree")
ml<-midpoint(ml)
ggtree(ml) %<+% relocate(m429,Sample) +
    geom_tippoint(aes(fill=Species), pch=21) +
  geom_tiplab(align = TRUE, size=1, aes(label=`Tributary/Collection Location`))
ggsave("outputs/108/ml-429.tree.pdf", width=14, height=20)
```
# Downsample snyderi
