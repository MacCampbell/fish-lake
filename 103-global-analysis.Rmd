...
```{r}
dat<-read.dna(file="outputs/103/pruned.min4.phy")
write.nexus.data(dat, file="outputs/103/pruned.nex")
```

Let's make a network in SplitsTree

```{r}
net<-read.nexus.networx("outputs/103/pruned.min4.network")
```
...
n<-g + 
  geom_tippoint(aes(fill=Species), cex=3, pch=21, alpha=0.75) +
  scale_fill_viridis_d(option="H") +
  guides(fill = guide_legend(override.aes = list(pch = 22)))

n

ggsave("outputs/103/network.pdf")
```
```{r}
df<-relocate(million, Sample) %>% rename(label=Sample)

t<-read.nexus(file = "outputs/103/nj.tre")
t<-midpoint_root(t) 
ggtree(t) %<+% df +
  
  geom_tippoint(aes(fill=Species), pch=21) +
  geom_tiplab(aes(label=paste0(Species," ",Vicinity," ",`Tributary/Collection Location`), x=1200), size=1) +
  scale_fill_viridis_d(option="H") +
  guides(fill = guide_legend(override.aes = list(pch = 22))) +
  geom_treescale() +
  xlim(0,1500)

ggsave("outputs/103/nj-tree.pdf", height=30, width = 8)
```